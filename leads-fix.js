  ...
  document.getElementById('leadsTableBody').innerHTML = leads.length ? leads.map(l => `
    <tr>
      ...
      <td><div style="display:flex;gap:6px;">
        ...
        <button class="btn btn-sm btn-ghost btn-icon" title="Excluir" onclick="confirmarExcluirLead('${l.id}', '${(l.nome||'esse lead').replace(/'/g, "\\'")}')"><i class="ti ti-trash" style="color:#d9534f;"></i></button>
      </div></td>
    </tr>`).join('')
    : '<tr><td colspan="7" style="text-align:center;padding:32px;color:var(--text-secondary);">Nenhum lead encontrado no período</td></tr>';
};

// ⚠️ AJUSTE 28/07: exclusão de lead com confirmação — pedido do Jean pra
// dar uma limpada na lista sem correr risco de apagar sem querer.
async function confirmarExcluirLead(leadId, nomeLead) {
  const confirmado = confirm(`Tem certeza que deseja excluir "${nomeLead}"?\n\nIsso não pode ser desfeito.`);
  if (!confirmado) return;
  try {
    const database = (typeof db !== 'undefined') ? db : (window.supabaseClient || window.sb);
    const { error } = await database.from('leads').delete().eq('id', leadId);
    if (error) throw error;
    if (typeof STATE !== 'undefined' && Array.isArray(STATE.leads)) {
      STATE.leads = STATE.leads.filter(l => l.id !== leadId);
    }
    if (typeof toast === 'function') toast('Lead excluído.', 'success');
    if (typeof renderLeads === 'function') renderLeads();
  } catch (e) {
    console.error('[excluir lead]', e);
    const mensagemAmigavel = String(e.message || e).includes('foreign key')
      ? 'Não foi possível excluir — esse lead tem consulta, orçamento ou mensagem vinculada. Cancele/remova esses registros primeiro, ou mude o status pra "cancelado" em vez de excluir.'
      : 'Erro ao excluir: ' + (e.message || e);
    if (typeof toast === 'function') toast(mensagemAmigavel, 'error');
    else alert(mensagemAmigavel);
  }
}
