export default async function handler(req, res) {
  ...
  try {
    ...
    const clinic_id = parts[0] || null;
    const user_id = parts[1] || null;
    const plano = parts[2] || 'clinica';
    const periodo = parts[3] || 'mensal';
    const meses = { mensal: 1, semestral: 6, anual: 12 };
    const agora = new Date();
    const fim = new Date(agora);
    fim.setMonth(fim.getMonth() + (meses[periodo] || 1));
    // Busca por user_id
    const existeResp = await fetch(
      `${SUPABASE_URL}/rest/v1/assinaturas?user_id=eq.${user_id}&order=created_at.desc&limit=1`,
      { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
    );
    const existe = await existeResp.json();
    if (existe?.length > 0) {
      await fetch(`${SUPABASE_URL}/rest/v1/assinaturas?user_id=eq.${user_id}`, {
        method: 'PATCH',
        headers: {
          'apikey': SUPABASE_KEY,
        ...
        body: JSON.stringify({
          plano, periodo, status: 'active',
          expires_at: fim.toISOString(),
      ...
      await fetch(`${SUPABASE_URL}/rest/v1/assinaturas`, {
        ...
        body: JSON.stringify({
          clinic_id, user_id, plano, periodo, status: 'active',
          expires_at: fim.toISOString(),
          mp_payment_id: String(data.id),
        }),
      });
